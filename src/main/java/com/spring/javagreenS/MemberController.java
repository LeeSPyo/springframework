...
public class MemberController {
	...
	public String memLoginGet(HttpServletRequest request) {
		// 로그인폼 호출시 기존에 저장된 쿠키가 있다면 불러와서 mid에 담아 넘겨준다.
...
//			 회원 인증처리된경우에 수행할 내용들을 기술한다.(session에 저장할 자료처리, 쿠키값처리...)
			String strLevel = "";
...
//			session.setAttribute("sLastDate", vo.getLastDate());
			...
			// 방문횟수(오늘방문횟수) 누적하기(최종 접속일 처리) - service객체에서 처리
...
//			redirect.addAttribute("mid", mid);
	...
	// 아이디 / 비밀번호 찾기를 위한 '임시 비밀번호 발급'해 주기( 폼띄우기 )
	...
	// 아이디 / 비밀번호 찾기를 위한 '임시 비밀번호 발급'해 주기 ( 처리 )
...
//			메세지 보관함의 내용을 편집해서 다시 보관함에 담아둔다.
			content = "<hr/>신규 비밀 번호는 : <font color='red'><b>" + content + "</b></font>";
			content += "<br><hr>아래 주소로 다시 로그인하셔서 비밀번호를 변경하시기 바랍니다.<hr><br>";
			content += "<p><img src=\"cid:main.jpg\" width='500px'></p><hr>";
			content += "<p>방문하기 : <a href='http://192.168.50.132:9090/javagreenS'>javagreenS사이트</a></p>";
			content += "<hr>";
			
			messageHelper.setText(content, true);
			
			// 본문에 기재된 그림파일의 경로를 따로 표시시켜준다.
			FileSystemResource file = new FileSystemResource("C:\\JavaGreen\\springframework\\works\\javagreenS\\src\\main\\webapp\\resources\\images\\main.jpg");
